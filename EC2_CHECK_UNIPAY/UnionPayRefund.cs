    ...
    class UnionPayRefund : UnionPaySetting
    {
        ...
        public void updateUnionPayRefund()
        {
            ...
            { 
                ...
                uc.purchAmt = dr["RTN_AMOUNT"].ToString();//設置退貨金額
                ...
                {   
                    InsertLog.insertLog("發動退貨失敗訂單編號:" + dr["OrderId"].ToString() + " 回應値:" + rtnCode);
                    InsertLog.insertLog("發動退貨失敗訂單編號:" + dr["OrderId"].ToString() + " 回應訊息 = " + uc.outPara.respCode);
                    InsertLog.insertLog("發動退貨失敗訂單編號:" + dr["OrderId"].ToString() + " 回應訊息 = " + uc.outPara.respMsg);
